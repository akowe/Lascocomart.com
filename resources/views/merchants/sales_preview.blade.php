                        ...
                        <div class="col-md-6 col-lg-6 d-flex">
                              <div class="card border-0 bg-primary text-white text-center mb-grid w-100">
                                    <div class="d-flex flex-row align-items-center h-100">
                                          ...
                                          <span class="card-body text-white" style="">
                                          ...
                                          <table class="table-striped table" id="table">
                                                ...
                                                <tbody>
                                                      ...
                                                      <tr class="small">
                                                            ...
                                                            <td><a href="order_invoice/{{ $product->order_number }}"
                                                                              title="Click to view">{{$product->order_number }}</a></td>
                                                            <td>{{$product->prod_name }}</td> 
                                                            <td>{{$product->order_quantity }}</td>
                                                            <td>{{number_format($product->seller_price ) }}</td>
                                                            <td>{{ $product->status}}</td>
                                                            <td><span class="text-success"> {{ $product->delivery_status}}</span>
                                                                @if(empty($product->delivery_status) )
                                                                <form action="{{ url('order-delivery') }}/{{ $product->id}}"
                                                                        method="POST">
                                                                        @csrf
                                                                        <input type="hidden" name="order_id" id=""
                                                                              value="{{ $product->id}}">
                                                                        <button class="btn btn-danger btn-xs"
                                                                              type="submit">Confirm Delivery</button>
                                                                  </form>
                                                                @endif
                                                                </td>
                                                      </tr> 
